/**
 * This file is originally from Carnegie Mellon University from the project Graphlab.
 *
 * The original file is /graphlab/src/graphlab/engine/warp_engine.hpp
 *
 * It is modified by me (Jinka Sai Sagar) to suit my usecase. This
 * file helps in understanding the integration of Apache Mesos and Graphlab.
 *
 * Some of the fnctions are added and few are edited. 
 *
 * Let us take the example of page rank program
 * and see how to make use of this file. After submitting the graph i.e. finalizing, we write vertex 
 * program. Then we call engine.start(). engine.start() calls begin() in this file and one can see
 * the flow by looking into the code.
 *
 * If any issues please contact "jsaisagar@gmail.com". I am looking forward to help.
 */
...
//	using mesos::internal::logging::Flags;
namespace warp {
   ...
   * \ref graphlab::warp_engine_edited::signal or
   * \ref graphlab::warp_engine_edited::signal_all.  In either case all
   * machines should invoke signal or signal all at the same time.  Finally,
   * computation is initiated by calling the
   * \ref graphlab::warp_engine_edited::start function.
   *
   ...
   * typedef graphlab::warp_engine_edited<graph_type> engine_type;
   *
  ...
  public:
    ...
    //extern warp_engine_edited *edit;
    struct context {
      typedef warp_engine_edited engine_type;
      typedef typename engine_type::graph_type graph_type;
      typedef typename graph_type::vertex_type vertex_type;
      typedef typename graph_type::edge_type edge_type;
      typedef typename graph_type::local_vertex_type local_vertex_type;

      warp_engine_edited& engine;
      graph_type& graph;
      std::string original_value;
      vertex_type vtx;
      bool vtx_set;

      context(warp_engine_edited& engine, graph_type& graph, 
              vertex_type vtx):
          engine(engine), 
          graph(graph), 
          vtx(vtx),
  ...
  //private:
    ...
    friend class resourceCalculation;
    ...
    /// The RPC interface
    dc_dist_object<warp_engine_edited> rmi;
  ...
  public:
     ...
     */
    warp_engine_edited(distributed_control &dc,
                            graph_type& graph,
 ...
 **************************************************************************/
  ...
  private:
    ...
    void internal_signal_gvid(vertex_id_type gvid,
      ...
      if (graph.is_master(gvid)) {
        ...
        rmi.remote_call(proc, &warp_engine_edited::internal_signal,
                        gvid, message);
      }
    } 
    ...
    void internal_stop() {
      for (procid_t i = 0;i < rmi.numprocs(); ++i) {
        rmi.remote_call(i, &warp_engine_edited::rpc_internal_stop);
      }
    }
  ...
  public:
     ...
     *
     * \code
     * graphlab::warp_engine_edited<graph_type> engine(dc, graph, opts);
  ...
  //private: 
	...
	//logstream(LOG_INFO) << "In get_next function\n ";
	//logstream(LOG_INFO) << "Value of stat is " <<stat<<std::endl;
    ...
    bool try_to_quit(size_t threadid,
      ...
      if (stat == sched_status::EMPTY || force_stop) {
        ...
        for (procid_t i = 0;i < rmi.dc().numprocs(); ++i) {
          rmi.remote_call(i, &warp_engine_edited::set_endgame_mode);
        } 
    ...
    void synchronize_one_vertex(vertex_type vtx) {
      ...
      foreach(procid_t mirror, lvtx.mirrors()) {
        rmi.remote_call(mirror, &warp_engine_edited::update_vertex_value, vtx.id(), vtx.data());
      }
    }
    ...
    void synchronize_one_vertex_wait(vertex_type vtx) {
      ...
      foreach(procid_t mirror, lvtx.mirrors()) {
        futures.push_back(object_fiber_remote_request(rmi, 
                                                      mirror, 
                                                      &warp_engine_edited::update_vertex_value, 
                                                      vtx.id(), 
                                                      vtx.data()));
      }
	...
	logstream(LOG_INFO)<<"in evaluation \n";
	...
	logstream(LOG_EMPH) << "In eval function\n ";
     ...
     */
    /*void thread_start(size_t threadid, sched_status::status_enum status) {
      ...
      while(1) {
        ...
        sched_status::status_enum stat = status;
...
void thread_start(size_t threadid) {
      ...
      std::vector<std::vector<lvid_type> > internal_lvid;
      lvid_type sched_lvid;
      //logstream(LOG_EMPH) << "In thread function "<<threadid<<std::endl;;
      message_type msg;
      float last_aggregator_check = timer::approx_time_seconds();
      timer ti; ti.start();
      while(1) {
...
while(!aggregation_queue[fiber_control::get_worker_id()].empty()) {
        ...
        if (stat != sched_status::EMPTY) {
          eval_sched_task(sched_lvid, msg);
	  logstream(LOG_INFO)<<"Status is not empty\n";
          if (endgame_mode) rmi.dc().flush();
        }
        else if (!try_to_quit(threadid, has_sched_msg, sched_lvid, msg)) {
          /*
 ...
 *                       */
          if (has_sched_msg) {
	    logstream(LOG_INFO)<<"Status is empty unfortunately\n";
            eval_sched_task(sched_lvid, msg);
          }
        } else {
          break;
...
void begin(){
      ...
      executorInfo.mutable_command()->set_value(uri);
      executorInfo.set_name("Graphlab Executor");
      executorInfo.set_source("graphlab.cpp");


      mesos::FrameworkInfo frameworkInfo;
      frameworkInfo.set_user("");//Generally mesos fills this user part.
      frameworkInfo.set_name("Graphlab framework");
        //Here the context of the engine is passed.
      pointer<engine_type> point(this,executorInfo); 
      logstream(LOG_INFO) << "We are in engine before the driver is initiated " <<std::endl; 
      resourceCalculation calculated;
	//calculated.calculation(allocatedmem,ncpus);
	resourceCalculation::dirty<engine_type> dirt;
	dirt.set_pointer(this);
	//Mesos Scheduler Driver is initialized here.
 	MesosSchedulerDriver driver(&point.myscheduler,frameworkInfo, cool);
	driver.run();
	logstream(LOG_INFO) << "We are in engine after the driver is initiated " <<std::endl;
//	start();
	next();
      }
	template<typename horrible>
	void threading(size_t i, horrible *horrified)
	{
	//	logstream(LOG_EMPH) << "In threading function "<<i<<std::endl;
		thrgroup.launch(boost::bind(&engine_type::thread_start,horrified,i));
	}
     void start()
	{
//		bool old_fasttrack = rmi.dc().set_fast_track_requests(false);
...
execution_status::status_enum next()
	{
		logstream(LOG_INFO) << " I am in next function which is called after the driver is aborted " <<std::endl;
		bool old_fasttrack = rmi.dc().set_fast_track_requests(false);
		thrgroup.join();
		//logstream(LOG_INFO) << " I am in next function which is called after the driver is aborted " <<std::endl;
      		//aggregator.stop();
		...
		logstream(LOG_INFO) << "1111111111111111111111111111";
		size_t ctasks = programs_executed.value;
		logstream(LOG_INFO) << "222222222222222222222222222";
      		rmi.all_reduce(ctasks);
		logstream(LOG_INFO) << "33333333333333333333333333333";
    ...
    //execution_status::status_enum start( sched_status::status_enum stat) 
...
//#include </home/test/GRAPHLABS/graphlab/src/graphlab/engine/graph.hpp>
#endif 
